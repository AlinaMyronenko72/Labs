...
public class Collection<T extends Product> {
    private T[] product;
    ...
    public void clone(Collection<T> clon) {
        for (int i = 0; i < clon.count(); i++) {
            if (clon.getProduct(i).getClass() == Accessory.class) {
                Accessory a = new Accessory();
                a.clone((Accessory) clon.getProduct(i));
                this.add((T)a);
            } else {
                Clothes a = new Clothes();
                a.clone((Clothes) clon.getProduct(i));
                this.add((T) a);
            }
        }
        /*for(int i=0;i<clon.count();i++){
            this.add(clon.getProduct(i));
        }*/
    }
